...
function setup() {
  // put setup code here
  createCanvas(iphoneWidth / 2, iphoneHeight / 3);
  background(116, 158, 168);
  noStroke();
  fill(255, 0, 0);
  rectMode(CENTER);
  imageMode(CENTER);
  ...
  for(var i=0;i<1;i++){
  bee1Xarray[i]=random(160,160)}
  for(var i=0;i<1;i++){
  bee1Yarray[i]=random(330,330)}
}
...
image(backgroundLogo, width / 2, height / 2, width, height);
  ...
  for(var i=0;i<1;i++){
  image(bee, bee1Xarray[i],bee1Yarray[i], width/12, height/18);
